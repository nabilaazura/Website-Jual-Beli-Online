...
class Profil extends CI_Controller
{
	...
	public function sportstuff()
	{
		$data['judul'] = "List Perlengkapan Olahraga";
		$data['user'] = $this->User_model->getBy();
	...
	public function keranjang($id)
	{
		...
		if ($this->form_validation->run() == false) {
            ...
            if ($stok < $jum) {
                $this->session->set_flashdata('message', '<div class="alert alert-warning" role="alert">Stok tidak mencukupi!</div>');
			    redirect('Profil/keranjang/'.$id);
            } else {
                $data = [
                    'id_user' => $this->session->userdata('id'),
                    'id_sportstuff' => $this->input->post('id'),
                    'jumlah' => $this->input->post('jumlah'),
                    'total' => $this->input->post('jumlah') * $this->input->post('harga'),
                    'tanggal' => $this->input->post('tanggal'),
                ];
                $this->Keranjang_model->insert($data);
                $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Data Sportstuff Berhasil Ditambahkan ke keranjang!</div>');
	...
	{
		$jumlah_beli = count($this->input->post('sportstuff'));
		$data_p = [
		...
		for ($i = 0; $i < $jumlah_beli; $i++) {
			array_push($data_detail, ['id_sportstuff' => $this->input->post('sportstuff')[$i]]);
		...
		if ($this->Penjualan_model->insert($data_p, $upload_image) && $this->Detail_model->insert($data_detail)) {
			for ($i = 0; $i < $jumlah_beli; $i++) {
				$this->Sportstuff_model->min_stok($data_detail[$i]['jumlah'], $data_detail[$i]['id_sportstuff']) or die('gagal min stok');
			}
